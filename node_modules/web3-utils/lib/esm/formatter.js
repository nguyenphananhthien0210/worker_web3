...
export const convertScalarValue = (value, ethType, format) => {
    try {
        ...
        if (baseType === 'bytes') {
            switch (format.bytes) {
                case FMT_BYTES.HEX:
                    return bytesToHex(bytesToBuffer(value));
                case FMT_BYTES.BUFFER:
                    return bytesToBuffer(value);
                case FMT_BYTES.UINT8ARRAY:
                    return new Uint8Array(bytesToBuffer(value));
                default:
...
export const convert = (data, schema, dataPath, format, oneOfPath = []) => {
    ...
    if (!isObject(data) && !Array.isArray(data)) {
        return convertScalarValue(data, schema === null || schema === void 0 ? void 0 : schema.eth, format);
    }
    const object = data;
        ...
        if (Array.isArray(value)) {
            ...
            if (isObject(_schemaProp.items) && !isNullish(_schemaProp.items.eth)) {
                for (let i = 0; i < value.length; i += 1) {
                    ...
                    (_a = _schemaProp === null || _schemaProp === void 0 ? void 0 : _schemaProp.items) === null || _a === void 0 ? void 0 : _a.eth, format);
                }
                dataPath.pop();
                continue;
            }
            ...
            if (Array.isArray(_schemaProp === null || _schemaProp === void 0 ? void 0 : _schemaProp.items)) {
                for (let i = 0; i < value.length; i += 1) {
                    object[key][i] = convertScalarValue(value[i], _schemaProp.items[i].eth, format);
                }
                dataPath.pop();
                continue;
            }
        }
        object[key] = convertScalarValue(value, schemaProp.eth, format);
        dataPath.pop();
    }
    return object;
};
