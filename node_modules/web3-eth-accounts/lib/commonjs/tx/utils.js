...
const getAccessListData = (accessList) => {
    ...
    if ((0, types_1.isAccessList)(accessList)) {
        ...
        for (let i = 0; i < accessList.length; i += 1) {
            ...
            const addressBuffer = (0, utils_1.toBuffer)(item.address);
            const storageItems = [];
            // eslint-disable-next-line @typescript-eslint/prefer-for-of
            for (let index = 0; index < item.storageKeys.length; index += 1) {
                storageItems.push((0, utils_1.toBuffer)(item.storageKeys[index]));
            }
        ...
        for (let i = 0; i < bufferAccessList.length; i += 1) {
            ...
            const address = (0, utils_1.bufferToHex)(data[0]);
            const storageKeys = [];
            // eslint-disable-next-line @typescript-eslint/prefer-for-of
            for (let item = 0; item < data[1].length; item += 1) {
                storageKeys.push((0, utils_1.bufferToHex)(data[1][item]));
            }
            const jsonItem = {
                address,
                storageKeys,
...
const getAccessListJSON = (accessList) => {
    ...
    for (let index = 0; index < accessList.length; index += 1) {
        ...
        const JSONItem = {
            // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/consistent-type-assertions
            address: `0x${(0, utils_1.setLengthLeft)(item[0], 20).toString('hex')}`,
            storageKeys: [],
        };
        ...
        for (let slot = 0; slot < storageSlots.length; slot += 1) {
            ...
            JSONItem.storageKeys.push(`0x${(0, utils_1.setLengthLeft)(storageSlot, 32).toString('hex')}`);
        }
