...
import { bytesToBuffer, bytesToHex, hexToBytes, numberToHex, randomBytes, sha3Raw, toChecksumAddress, utf8ToHex, uuidV4, } from 'web3-utils';
...
export const recover = (data, signatureOrV, prefixedOrR, s, prefixed) => {
    ...
    const ecPublicKey = recoverPublicKey(Buffer.from(hashedMessage.substring(2), 'hex'), Buffer.from(signatureOrV.substring(2, V_INDEX), 'hex'), parseInt(v, 16) - 27, false);
...
export const encrypt = (privateKey, password, options) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (kdf === 'pbkdf2') {
        ...
        derivedKey = pbkdf2Sync(bufferPassword, Buffer.from(salt), kdfparams.c, kdfparams.dklen, 'sha256');
    }
    else if (kdf === 'scrypt') {
        kdfparams = {
        ...
        derivedKey = scryptSync(bufferPassword, Buffer.from(salt), kdfparams.n, kdfparams.p, kdfparams.r, kdfparams.dklen);
    }
    else {
        throw new InvalidKdfError();
    }
    const cipher = yield createCipheriv(privateKeyBuffer, Buffer.from(derivedKey.slice(0, 16)), initializationVector, 'aes-128-ctr');
...
export const privateKeyToAccount = (privateKey, ignoreLength) => {
    ...
    return {
        ...
        sign: (data) => sign(typeof data === 'string' ? data : JSON.stringify(data), privateKeyBuffer),
        encrypt: (password, options) => __awaiter(void 0, void 0, void 0, function* () { return encrypt(privateKeyBuffer, password, options); }),
    };
};
/**
 *
...
export const decrypt = (keystore, password, nonStrict) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (json.crypto.kdf === 'scrypt') {
        ...
        const bufferSalt = typeof kdfparams.salt === 'string'
            ...
            : kdfparams.salt;
        derivedKey = scryptSync(bufferPassword, bufferSalt, kdfparams.n, kdfparams.p, kdfparams.r, kdfparams.dklen);
    }
    ...
    const seed = yield createDecipheriv(Buffer.from(json.crypto.ciphertext, 'hex'), derivedKey.slice(0, 16), Buffer.from(json.crypto.cipherparams.iv, 'hex'));
